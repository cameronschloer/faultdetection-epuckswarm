...
def generate(seed, length, id_of_faulty, task, fault):
    ...
    <loop_functions library="build/loop_functions/homswarm_loop_functions/libhomswarm_loop_functions.so"
        ...
        <params  output="data/hom_data/{task}/{fault}/nohup_{task}_{fault}_{seed}.txt" 
            arenalength="3.0"/>
    ...
    with open(f"experiments/hom_experiments/{task}/{fault}/epuck_{task}_{fault}_{seed}.argos", 'w') as out:
        out.write(content)
