...
def generate(seed, length, id_of_faulty, task, fault):
    ...
    if task == "SWARM_FORAGING":
        ...
        <loop_functions library="build/loop_functions/foraging_loop_functions/libforaging_loop_functions.so"
            ...
            <foraging items="1"
                radius="0.1"
                arenalength="3.0"
                output="data/replication_data/{task}/{fault}/nohup_{task}_{fault}_{seed}.txt" />
        </loop_functions>
        ...
        <loop_functions library="build/loop_functions/homswarm_loop_functions/libhomswarm_loop_functions.so"
            ...
            <params  output="data/replication_data/{task}/{fault}/nohup_{task}_{fault}_{seed}.txt" 
                arenalength="3.0"/>
    ...
    with open(f"experiments/replication_experiments/{task}/{fault}/epuck_{task}_{fault}_{seed}.argos", 'w') as out:
        out.write(content)
